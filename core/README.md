...
If Amazon was split across multiple jobs/sections — the basin is normally run as the six `data/boundaries/Amazon_ACA/Amazon_ACA_{1..6}.geojson` subregions — concatenate on your local machine:
...
**Always concatenate with this script, not with `ogrmerge`, `cat`, or a plain `pd.concat`.** Tiles that straddle a subregion boundary are generated by *both* adjacent runs, and chips are cut across the whole tile, so those tiles emit the same detections twice. The script deduplicates on rounded patch centroid, keeping the highest confidence.

The double-counting is confined to a band one tile wide (0.05°) along each seam, but it is not cosmetic. A duplicate sits at distance zero from its twin, which depresses the k-th-nearest-neighbour distance `postprocess.py` uses to judge isolation — so a pair can slip past the stricter `t_iso` cutoff that should have rejected it. **Dedupe before postprocessing, never after.** Measured on 2023: 875 duplicate records (0.48%), plus 11 locations at `t0.43` and 3 at `t0.55` that survived the isolation test only because their twin was sitting on top of them.

Passing a single path deduplicates that file on its own, which is how already-concatenated outputs were repaired.
