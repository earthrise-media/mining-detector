...
The script deduplicates on rounded patch centroid, keeping the highest confidence. About half of duplicate pairs differ slightly in score (median delta 5e-4), the two subregions having been run separately, and taking the maximum matches what the threshold would have done had the location appeared once. The logic lives in `postprocess.py` (`dedupe_detections`, `centroid_key`); `concatenate.py` is the CLI over it.
...
Passing a single path deduplicates that file on its own. That is how the existing archive was repaired in August 2026 — 7,474 duplicates removed across 14 periods, with the originals kept alongside as `*-undeduped-todelete/`.
