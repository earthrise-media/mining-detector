...
def cmds_publish(_periods: Sequence[str]) -> List[str]:
    ...
    return [
        ...
        "#",
        "#    They expire. A long upload can fail part-way with an auth error, in",
        "#    which case re-copy them and re-run -- cp skips nothing, but it",
        "#    overwrites rather than duplicating, so re-running is safe.",
        "#",
        "#    Note cp does not delete: anything removed from the staging tree",
        "#    stays on the bucket until it is deleted by hand.",
        f"aws s3 cp --recursive data/staging_source-coop/ {coop}/",
