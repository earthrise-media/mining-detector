...
def deploy_tpa_nn(dlkey, product_id, model_name, year):
    ...
    def get_imagery(dlkey, year):
        ...
        sd = datetime(year, 3, 1)
        ed = datetime(year, 9, 1)
    ...
    # grab all imagery
    imagery, raster_info = get_imagery(dlkey, year)
    ...
    for index, geometry in enumerate(tile_coords):
        ...
        if pred_dict['mean'][index][1].astype('float') > 0.01:
...
def get_product(create_product, product_id, product_name, product_desc):
    if create_product:
        ...
        product = dl.vectors.FeatureCollection.create(
                            ...
                            title=product_name,
                            description=product_desc)
        print('Deleted old product and created new product')
...
def main(args):
    ...
    for dlkey in tqdm(tiles):
        task = async_func(dlkey, product_id=product.id, model_name=args.model_name, year=args.year)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--year', type=int, help='What year to evaluate',
                        default=2020)
